...
def scheduler_test2():
    ...
    print(f'Шедулером выполнена заглушка компарекоунтпос в {datetime.datetime.now()}')
...
def report_for_test1():
    ...
    if form.validate_on_submit():
        ...
        # scheduler.add_job(id=f'job_{session["task_name"]}',func=scheduler_test2(),)
...
if __name__ == '__main__':
    ...
    scheduler.add_job(id='no_web_test1', func=scheduler_test1, args=('ARRR',), trigger='cron', minute="*")
    scheduler.init_app(app) #не удалось выполнить задачи при исполнении данной строки кода
    scheduler.start()
    # print(app.config)
    app.run(debug=True, use_reloader=False) # use_reloader позволяет выполнить задачу без дублей ( при необходимости см. описание параметра)
